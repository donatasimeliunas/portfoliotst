...
import { videographyFilms } from "@/content/videography"

export default function VideographyLt() {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <h1 className="text-3xl font-bold mb-6">Mielos akimirkos</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {videographyFilms.map((film) => {
          const localeData = requireLocale(film.locales, "lt")
          return (
            <Link href={`/lt/videography/${film.id}`} key={film.id} className="block">
